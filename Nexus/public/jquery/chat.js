...
jQuery("<div style='bottom: 0px; right: 20px;' />" ).attr("id","chatbox_"+chatboxtitle).addClass("chatbox").html('<div class="chatboxhead"><div class="chatboxtitle">'+chatboxtitle.split("_").join(".")+'</div><div class="chatboxoptions"><a href="javascript:void(0)" onclick="javascript:toggleChatBoxGrowth(\''+chatboxtitle+'\')">-</a> <a href="javascript:void(0)" onclick="javascript:closeChatBox(\''+chatboxtitle+'\')">X</a></div><br clear="all"/></div><div class="chatboxcontent"></div><div class="chatboxinput"><textarea class="chatboxtextarea" onkeydown="javascript:return checkChatBoxInputKey(event,this,\''+chatboxtitle+'\');"></textarea></div>').appendTo(jQuery( "body" ));
...
function chatHeartbeat(){
		...
		for (x in newMessagesWin) {
			if (newMessagesWin[x] == true) {
				...
				if (blinkNumber >= blinkOrder) {
					document.title = x.split("_").join(".")+' says....';
					titleChanged = 1;
					break;	
				}
			}
	...
	jQuery.ajax({
	  url: baseUrl+"chat.php?action=chatheartbeat&user="+userNAME,
	  cache: false,
	  dataType: "json",
		...
		jQuery.each(data.items, function(i,item){
			if (item)	{ // fix strange ie bug
				...
				if (item.s == 2) {
					...
					jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxmessagefrom">'+item.f.split("_").join(".")+':&nbsp;&nbsp;</span><span class="chatboxmessagecontent">'+item.m+'</span></div>');
				}
...
function closeChatBox(chatboxtitle) {
	...
	jQuery.post(baseUrl+"chat.php?action=closechat&user="+userNAME, { chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			jQuery.post(baseUrl+"chat.php?action=sendchat&user="+userNAME, {to: chatboxtitle, message: message} , function(data){
				message = message.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\"/g,"&quot;");
				jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxmessagefrom">'+username.split("_").join(".")+':&nbsp;&nbsp;</span><span class="chatboxmessagecontent">'+message+'</span></div>');
				//jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").scrollTop(jQuery("#chatbox_"+chatboxtitle+".chatboxcontent")[0].scrollHeight);
			});
		}
		chatHeartbeatTime = minChatHeartbeat;
...
function startChatSession(){  
	...
	jQuery.ajax({
	 // url: "http://nexus.php-dev.in/application/views/chat.php?action=startchatsession",
	 url: baseUrl+"chat.php?action=startchatsession&user="+userNAME,
