<script>
 $(document).ready(function() {
    ...
    $(".status").live("click", function(){
        ...
        $.ajax({
                type: "POST",
                dataType: "json",
                url: baseURL+"admin/travel_locations/set_status/"+id,
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: baseURL+"admin/travel_locations/unset_status/"+id,
                success: function(response) {
                    // success function is called when data came back
                    // for example: get your content and display it on your site
                    //alert(response);
 ...
 <div id="main_content">
			...
			<select id="table_name" class="table_name" style="width:30%;">
				<option value='travel_locations' selected>Travels</option>
				<option value='hotel_locations'>Hotels</option>
				<option value='guesthouse_locations'>Guest House</option>
			</select>

            <h2 class="grid_12">Travel Locations
                <div style="float:right;margin-right:5%;">
                    <a href="<?php echo site_url('admin/travel_locations/add') ?>"><font size=2>Add New</font></a>
                </div></h2>
            <div class="grid_12">
                <div class="box">
                    <div class="header">
                        <img src="<?php echo base_url(); ?>design/Template/img/icons/packs/fugue/16x16/marker.png" width="16" height="16">
                        <h3>Travel Location</h3><span></span>
                    </div>
                    <div class="content">
                                ...
                                <?php foreach ($locationsDetails as $location): ?>
                                    <tr>
                                       <td><input type="checkbox" value="<?php echo $location['id']; ?>" name="delete[]" class="chkbox"></td>
                                        ...
                                        <td>
                                            <a title="Edit Item" href="<?php echo site_url('admin/travel_locations/edit/'.$location['id']); ?>"><img src='<?php echo base_url(); ?>/design/Template/img/icons/packs/diagona/16x16/019.png'/></a>&nbsp; | &nbsp;
                                            <a class="delete tooltip" href="javascript: void(0)'" title="Delete Item" onclick="javascript : deleteConfirm('<?php echo site_url("admin/travel_locations/delete/".$location['id']); ?>');"><img src='<?php echo base_url(); ?>/design/Template/img/icons/packs/diagona/16x16/101.png'/></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
...
<?php echo form_button(array('name'=>'mysubmit','id'=>'mysubmit'), 'Delete');?>
					   ...
					   <?php } else if(is_array($priv['travel_locations'])&& in_array(3,$priv['travel_locations'])){
						?>
                       <?php echo form_button(array('name'=>'mysubmit','id'=>'mysubmit'), 'Delete');?>
					   <?php } ?></form>
