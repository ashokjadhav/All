 ...
 $(document).ready(function() {
	 $(".status").live("click", function(){
        ...
        $.ajax({
                type: "POST",
                dataType: "json",
                url: baseURL+"admin/history_milestones/set_status/"+id,
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: baseURL+"admin/history_milestones/unset_status/"+id,
                success: function(response) {
                    // success function is called when data came back
                    // for example: get your content and display it on your site
                    //alert(response);
...
<div id="main_content">
            ...
            <h2 class="grid_12">Milestones
                <div style="float:right;margin-right:5%;">
                    <a href="<?php echo site_url('admin/history_milestones/add') ?>"><font size=2>Add Milestones</font></a>
                </div></h2>
            <div class="grid_12">
                <div class="box">
                    <div class="header">
                        <img src="<?php echo base_url(); ?>/design/Template/img/icons/packs/fugue/16x16/shadeless/report--pencil.png" width="16" height="16">
                        <h3>Milestones</h3><span></span>
                    </div>
                    <div class="content">
...
<form name="frmproducts" id="frmproducts" method="post">
                        <table id="table-example" class="table">
                            ...
                            <tbody>
                                <?php $x = 1; ?>
                                <?php foreach($milestonesDetails as $notice):?>

                                    <tr>
                                        <td><input type="checkbox" value="<?php echo $notice['id']; ?>" name="delete[]" class="chkbox"></td>
                                        <td><?php echo $x++; ?></td>
                                        <td><?php echo date('d F Y', strtotime($notice['date'])); ?></td>
                                        <td>
										<a href="#" class="view-answer" id="<?php echo $notice['id']; ?>">View</a>
<div class="dialog" id="dialog_<?php echo $notice['id']; ?>" title="<?php echo date('d F Y', strtotime($notice['date'])); ?>"  style="display:none;">
				<p><span style="font-family:verdana,geneva,sans-serif; font-size:14px"> <?php echo $notice['description']; ?></span></p>
                                  ...
                                  <input type="checkbox"  class="status" id="<?php echo  $notice['id'];?>" name="status" value='<?php echo  $notice['id'];?>'
                                    <?php if($notice['status']=='1'){?>checked<?php }?>>
                                        ...
                                        <td>
                                            <a title="Edit Item" href="<?php echo site_url('admin/history_milestones/edit/' . $notice['id'] ); ?>"><img src='<?php echo base_url(); ?>/design/Template/img/icons/packs/diagona/16x16/019.png'/></a>&nbsp; | &nbsp;

											<a class="delete tooltip" href="javascript: void(0)'" title="Delete Item" onclick="javascript : deleteConfirm('<?php echo site_url("admin/history_milestones/delete/".$notice['id']); ?>');"><img src='<?php echo base_url(); ?>/design/Template/img/icons/packs/diagona/16x16/101.png'/></a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
...
<?php echo form_button(array('name'=>'mysubmit','id'=>'mysubmit'), 'Delete');?>
					   ...
					   <?php } else if(is_array($priv['history_milestones'])&& in_array(3,$priv['history_milestones'])){
						?>
                       <?php echo form_button(array('name'=>'mysubmit','id'=>'mysubmit'), 'Delete');?>
					   <?php } ?></form>
