...
class Policies extends CI_Controller {
 ...
 /**
  ...
  if ($this->input->post('delete')) {
      $multiDelete = $this->policies_model->multi_delete_policies($this->input->post('delete'));
      if ($multiDelete)
        $this->session->set_flashdata('success', 'Policies Deleted Successfully !!');
      else
        $this->session->set_flashdata('error', 'Failed to Delete Policies !!');
    ...
    $arrData['policiesDetails'] = $this->policies_model->get_policies_details();
   $arrData['middle'] = 'admin/policies/policies';
  ...
  public function add() {
	...
	$this->form_validation->set_rules('status', 'Status', 'required');
		...
		if ($this->form_validation->run() == True) // passed validation proceed to post success logic
		{
           ...
           $inserted_rightsFlag = true;
            ...
            $insertedFlag = $this->policies_model->save_policiesdetails($arrData);
      if ($insertedFlag) {
    ...
    $arrData['tab']=$this->uri->segment(2);
    $arrData['middle'] = 'admin/policies/add';
    $this->load->view('admin/template', $arrData);
  }
  /**
   * edit
  ...
  public function edit($iuserId) {
    ...
    if ($_POST) {
      ...
      $policies_details = $this->policies_model->get_policies_details($iuserId);
 if ($this->input->post('submit')) {
	...
	$this->form_validation->set_rules('status', 'Status', 'required');
		...
		if ($this->form_validation->run() == TRUE) // passed validation proceed to post success logic
		{
          ...
          $UpdateData["modified_date"] = $date;
           ...
           $updateFlag = $this->policies_model->update_policies($iuserId, $UpdateData);
            ...
            if ($updateFlag) {
              ...
              redirect('admin/policies/edit/' . $iuserId);
            }
		}
   ...
   $arrData['tab']=$this->uri->segment(2);
    $arrData['middle'] = 'admin/policies/edit';
    $this->load->view('admin/template', $arrData);
  }
  /**
   * delete
  ...
  public function delete($iuserId) {
	  if(!check_priviledges()){
		$this->session->set_flashdata('error', 'You  are not authorize to access this resource!!');
       redirect('admin/dashboard');}
    $delete = $this->policies_model->delete_policies($iuserId);
    if ($delete)
      $this->session->set_flashdata('success', 'Policy Details Deleted Successfully !!');
    else
      $this->session->set_flashdata('error', 'Failed to Delete Policy Details!!');
      redirect('admin/policies');
  }
